@extends('admin.index')
  ...
  <div class="card-body">
    <div class="table-responsive">
      <form method="POST" action="{{ route('mempelai.store') }}" role="form" enctype="multipart/form-data">
        @csrf
          <div class="row">
            <!-- col 1  -->
            <div class="col"> 
            ...
            <div class="col">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="namawanita" readonly>
              </div>
              <div class="form-group">
                <label for="sel1">Pilih Galeri:</label>
                <button type="button" class="btn btn-info btn-sm text-white" data-toggle="modal" data-target="#getDataGaleriModal">
                  <i class="fa fa-female" aria-hidden="true"></i>
                </button>
              </div>
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="namagaleri" readonly>
              </div>
            </div>
          </div>
  ...
  <!-- Data Mempelai Galeri Modal-->
  <div class="modal fade" id="getDataGaleriModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl" role="document">
          <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="card-body">
                  <div class="table-responsive">
                    <table id="dataTableGaleri" class="table table-bordered" width="100%" cellspacing="0">
                      <thead>
                        <tr>
                          <th>Nama Pembuat</th>
                          <th>Kategori</th>
                      ...
                      <tbody>
                        <!-- menggrupkan gambar per id created_by -->
                        @foreach ($Galeri as $item)
                          <?php 
                            $id_create = $item['created_by']; 
                            $SqlGambar = "SELECT created_by, gambar 
                                            FROM galeri 
                                            WHERE created_by = '" . $id_create . "' 
                                          ";
                            $resultsGambar  = DB::select(DB::raw($SqlGambar));
                            $Galeriwhere = json_decode(json_encode($resultsGambar), true);
                            $id = '';
                            $content = "";
                            $content 	.= '<tr>';
                            $content 	.= '<td>' .$item['nama_pembuat']. '</td>';
                            $content 	.= '<td>' .$item['nama_kategori']. '</td>';
                            // $content 	.= '</tr>';
                            // $content 	.= '<tr>';
                            $content 	.= '<td>';
                            $idChange = false;
                          ?>
                          <?php 
                            foreach ($Galeriwhere as $itemm){ 
                              if ($id != '') {
                                if ($id == $itemm['created_by']) {
                                  $idChange = true;
                                }else {
                                  $idChange = false;
                                }
                              ...
                              // dd($itemm);
                              if(!$idChange) { 
                                foreach ($itemm as $value) {
                                  $content 	.= '<img src="' .$value. '" alt="" width="30%" class="img-thumbnail">';
                                }
                              }
                            }
                            $content 	.= '</td>';
                          ...
                          <?= $content; ?>
                            <td align="center"> 
                                <button class="btn btn-sm btn-primary pilih-galeri" value="{{ $item['created_by'] }}" data-dismiss="modal">
                                  <i class="fa fa-check" style="color:white" value="{{ $item['created_by'] }}"></i>
                                </button>
                            </td>
                          </tr>
                        @endforeach
